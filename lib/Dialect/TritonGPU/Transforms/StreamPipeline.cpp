...
class LoopPipeliner {
  ...
  /// Return an empty buffer of size <numStages-1, ...>
...
SmallVector<Value> LoopPipeliner::collectNewLoopArgs() {
  ...
  for (auto v : forOp.getIterOperands()) {
    newLoopArgs.push_back(lookupOrDefault(v, numStages - 1));
  }
...
void LoopPipeliner::prefetchNextIteration(scf::ForOp newForOp,
  ...
  DenseMap<Value,Value> curLoad;
  DenseMap<Value,Value> curMask;
  Value curIV;
  Value nextLoopCond;
  {
    OpBuilder::InsertionGuard g(builder);
    builder.setInsertionPointToStart(newForOp.getBody());
    curIV = newForOp.getRegionIterArgs()[ivIndex];
    ...
    for (Operation *op : orderedDeps) {
      ...
      if (validLoads.contains(op->getResult(0))) {
        ...
        Value newMask;
        Value loadVal;
        { // pre-load global -> regs at the top of loop
          // load from global -> stream regs
          //assert(input is block arg);
          newMask = getLoadMask(loadOp, nextMapping.lookupOrDefault(loadOp.getMask()),
