...
bool usePrintf(triton::gen::PrintOp op) {
  // Simply use printf if no operand or the operand is scalar.
  if (op.getNumOperands() == 0)
    return true;

  // tt.print is already decomposed to triton_gen.print per value.
...
struct PrintOpConversion : public ConvertOpToLLVMPattern<triton::gen::PrintOp> {
  using ConvertOpToLLVMPattern<triton::gen::PrintOp>::ConvertOpToLLVMPattern;

  LogicalResult
  matchAndRewrite(triton::gen::PrintOp op, OpAdaptor adaptor,
...
struct AssertOpConversion
    : public ConvertOpToLLVMPattern<triton::gen::AssertOp> {
  using ConvertOpToLLVMPattern<triton::gen::AssertOp>::ConvertOpToLLVMPattern;

  LogicalResult
  matchAndRewrite(triton::gen::AssertOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    auto loc = op.getLoc();
    auto ctx = rewriter.getContext();
    auto b = TritonLLVMOpBuilder(loc, rewriter);
...
struct DebugOpsToLLVM
  ...
  void runOnOperation() override {
    ...
    TritonGENToLLVMTypeConverter typeConverter(context, option);
