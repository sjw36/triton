...
using ExternElementwiseOp = triton::gen::ExternElementwiseOp;
...
struct ExternElementwiseOpConversion
    : public OpToVecLibConversion<triton::gen::ExternElementwiseOp> {
  using OpToVecLibConversion::OpToVecLibConversion;

  std::string getVecFnName(triton::gen::ExternElementwiseOp op,
...
struct MathToVecLibPass
    : public mlir::triton::gen::impl::MathToVecLibBase<MathToVecLibPass> {
  MathToVecLibPass() = default;
  // Default to 128-bit if no features are specified.
  size_t vec_size_in_bits = 128;

  explicit MathToVecLibPass(VecLib lib, std::set<std::string> gen_features) {
    this->lib = lib;
    update_vec_size(gen_features);
  }

  void update_vec_size(std::set<std::string> &gen_features) {
    // TODO:
    //  Refactor this as an independent function.
    //  And improve this to support other x86 SIMD ISAs and also for arm SVE
    //  (VLA)
    for (auto feature : gen_features) {
      if (feature == "avx512f") {
  ...
  void runOnOperation() override {
    ...
    if (!gen_features.empty()) {
      std::set<std::string> gen_features_set{gen_features.begin(),
                                             gen_features.end()};
      update_vec_size(gen_features_set);
    }
...
createMathToVecLibPass(VecLib lib, std::set<std::string> gen_features) {
  return std::make_unique<MathToVecLibPass>(lib, gen_features);
}
