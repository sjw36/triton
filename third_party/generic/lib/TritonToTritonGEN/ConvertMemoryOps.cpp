...
#include "generic/include/Analysis/TensorPtrShapeInfo.h"
...
struct LoadOpConversion : public MemoryOpConversion<triton::LoadOp> {
  ...
  lowerToContiguousRowMajor(triton::LoadOp loadOp,
    ...
    for (int64_t idx = 0; idx < numElems; idx += shape.back()) {
      ...
      Value memRef =
          rewriter.create<triton::gen::PtrToMemRefOp>(loc, memRefTy, ptr);
      Value vec;
      if (mask) {
        Value subMask = mask;
  ...
  LogicalResult lowerToGather(triton::LoadOp loadOp,
    ...
    auto gatherBase = rewriter.create<triton::gen::PtrToMemRefOp>(
...
struct StoreOpConversion : public MemoryOpConversion<triton::StoreOp> {
  ...
  LogicalResult lowerToScatter(triton::StoreOp storeOp,
    ...
    for (int64_t idx = 0; idx < numElems; idx += shape.back()) {
      ...
      auto scatterBase = rewriter.create<triton::gen::PtrToMemRefOp>(
...
struct GenStoreOpConversion : public MemoryOpConversion<triton::gen::StoreOp> {
  using MemoryOpConversion::MemoryOpConversion;

  LogicalResult
  matchAndRewrite(triton::gen::StoreOp storeOp, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    auto loc = storeOp.getLoc();
...
struct GenLoadOpConversion : public MemoryOpConversion<triton::gen::LoadOp> {
  using MemoryOpConversion::MemoryOpConversion;

  LogicalResult
  matchAndRewrite(triton::gen::LoadOp loadOp, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    auto loc = loadOp.getLoc();
...
public:
  explicit MemoryOpConversionTarget(MLIRContext &ctx) : ConversionTarget(ctx) {
    ...
    addIllegalOp<mlir::triton::gen::StoreOp, mlir::triton::gen::LoadOp>();
...
struct ConvertMemoryOps
    : public triton::gen::impl::ConvertMemoryOpsBase<ConvertMemoryOps> {
  ...
  void runOnOperation() override {
    ...
    TritonToTritonGENTypeConverter pointerConverter;
    RewritePatternSet patterns(context);
    patterns.add<LoadOpConversion, StoreOpConversion, GenStoreOpConversion,
                 GenLoadOpConversion>(axisInfoAnalysis, shapeInfoAnalysis,
