...
# for locating libTritonGENRuntime
try:
...
class GENUtils(object):
    ...
    def __new__(cls):
        if not hasattr(cls, "instance"):
            cls.instance = super(GENUtils, cls).__new__(cls)
        return cls.instance
...
static struct PyModuleDef ModuleDef = {{
  ...
  \"__triton_gen_launcher\",
  NULL, //documentation
  -1, //size
  ModuleMethods
}};

PyMODINIT_FUNC PyInit___triton_gen_launcher(void) {{
...
class GENLauncher(object):
    ...
    def __init__(self, src, metadata):
        ...
        mod = compile_module_from_src(src, "__triton_gen_launcher")
...
class GENDeviceInterface:
    ...
    def Event(self, enable_timing=True):
        if self.use_hooks:
            return GENDeviceInterface.HooksTimeAccessor(self)
        return GENDeviceInterface.TimerEvent()


class GENDriver(DriverBase):

    def __init__(self):
        self.utils = GENUtils()
        self.launcher_cls = GENLauncher
        super().__init__()
    ...
    def get_empty_cache_for_benchmark(self):
        ...
        # A typical LLC size for high-end server GENs are ~400MB.
        cache_size = 512 * 1024 * 1024
        return torch.empty(int(cache_size // 4), dtype=torch.int, device='gen')

    # TODO maybe GEN should do anything here
