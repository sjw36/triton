...
class GENOptions:
    ...
    # Zero value is used to utilize all available GEN cores.
    num_threads: int = 0
    ...
    # TODO: We may introduce GEN-specific options like # of cores.
    ukernels: str = None
    ...
    def get_ukernels(self) -> Ukernels:
        raw_ukernels = os.getenv("TRITON_GEN_UKERNELS_LIB", self.ukernels)
...
class GENBackend(BaseBackend):

    @staticmethod
    def supports_target(target: GPUTarget):
        return target.backend == "gen"
    ...
    def __init__(self, target: tuple) -> None:
        ...
        if 'amx-tile' in self.gen_features:
            if not gen.enable_amx():
                ...
                self.gen_features.discard('amx-int8')
                self.gen_features.discard('amx-fp16')
                self.gen_features.discard('amx-bf16')

    def parse_options(self, opts) -> Any:
        args = {k: opts[k] for k in GENOptions.__dataclass_fields__.keys() if k in opts}
        if "enable_fast_math" not in args:
            args["enable_fast_math"] = os.getenv("TRITON_GEN_FAST_MATH", "1") != "0"
        if "supported_fp8_dtypes" not in args:
            supported_fp8_dtypes = set(GENOptions.supported_fp8_dtypes)
    ...
    def get_module_map(self) -> Dict[str, ModuleType]:
        from triton.language.extra.gen import libdevice
    ...
    def make_so(src, metadata, options):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            lib_dirs = gen_driver.library_dirs
            libs = ["m", "TritonGENRuntime", "sleef"]
            so = _build("kernel", asm_path, tmpdir, lib_dirs, gen_driver.include_dirs, libs)
    ...
    @functools.lru_cache()
    def hash(self):
        # TODO: Get more detailed GEN info like raw brand name with supported ISAs.
