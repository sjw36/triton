...
#include "TritonGENToLLVM/Passes.h"
#include "TritonGENTransforms/Passes.h"
...
void init_triton_gen_passes_ttgenir(py::module &&m) {
  ...
  py::enum_<gen::VecLib>(m, "VecLib")
      .value("libsleef", gen::VecLib::Sleef)
      .value("libmvec", gen::VecLib::Mvec);

  py::enum_<gen::Ukernels>(m, "Ukernels")
      .value("OneDNN", gen::Ukernels::OneDNN)
      .value("XSMM", gen::Ukernels::XSMM);

  m.def("add_scalarize", [](mlir::PassManager &pm, bool skip_gather_scatter) {
    pm.addPass(
        mlir::triton::gen::createScalarizeUsingForOpPass(skip_gather_scatter));
  });
  m.def("add_convert_memory_ops", [](mlir::PassManager &pm,
                                     bool use_gather_scatter) {
    pm.addPass(mlir::triton::gen::createConvertMemoryOps(use_gather_scatter));
  });
  m.def("add_convert_ptr_ops", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createConvertPtrOps());
  });
  m.def("add_convert_elementwise_ops", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createConvertElementwiseOps());
  });
  m.def("add_convert_elem_manip_ops", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createConvertElemManipOps());
  });
  m.def("add_convert_dot_op", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createConvertDotOp());
  });
  m.def("add_convert_histogram_op", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createConvertHistogramOp());
  });
  m.def("add_convert_reduction_op",
        [](mlir::PassManager &pm, bool use_reduction_op,
           bool use_multidim_reduction_op) {
          pm.addPass(mlir::triton::gen::createConvertReductionOp(
              use_reduction_op, use_multidim_reduction_op));
        });
  m.def("add_convert_scan_op", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createConvertScanOp());
  });
  m.def("add_convert_cf_ops", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createConvertControlFlowOps());
  });
  m.def("add_convert_atomic_ops", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createConvertAtomicOps());
  });
  m.def("add_convert_debug_ops", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createConvertDebugOps());
  });
  m.def("add_triton_gen_canonicalizer", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createCanonicalize());
  });
  m.def("add_optimize_masks", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createOptimizeMasks());
  });
  m.def("add_convert_dot_product", [](mlir::PassManager &pm,
                                      bool useHorizontalSum) {
    pm.addPass(mlir::triton::gen::createConvertDotProduct(useHorizontalSum));
  });
  m.def("add_loop_invariant_code_motion", [](mlir::PassManager &pm) {
    pm.addPass(mlir::createLoopInvariantCodeMotionPass());
  });
  m.def("add_convert_dot_to_ukernels", [](mlir::PassManager &pm,
                                          gen::Ukernels ukernels) {
    pm.addPass(mlir::triton::gen::createConvertDotOpToUkernelOps(ukernels));
  });
  m.def("add_convert_dot_to_amx", [](mlir::PassManager &pm, bool convertInt8,
                                     bool convertFp16, bool convertBf16) {
    pm.addPass(mlir::triton::gen::createConvertDotToAMX(
        convertInt8, convertFp16, convertBf16));
  });
  m.def("add_convert_dot_to_fma", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createConvertDotToFMA());
  });
  m.def("add_convert_dot_generic", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createConvertDotGeneric());
  });
  m.def("add_convert_unsupported_ops",
        [](mlir::PassManager &pm, bool promote_bf16_to_fp32,
          ...
          pm.addPass(mlir::triton::gen::createConvertUnsupportedOps(
  ...
  m.def("add_decompose_fp_conversions",
        [](mlir::PassManager &pm, bool decomposeBf16Conversions,
          ...
          pm.addPass(mlir::triton::gen::createDecomposeFpConversions(
  ...
  m.def("add_lower_vector_multi_dim", [](mlir::PassManager &pm) {
    pm.addNestedPass<mlir::triton::FuncOp>(
        mlir::triton::gen::createLowerMultiReductionPass());
  });
  m.def("add_func_op_to_llvmir", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createFuncOpToLLVMPass());
  });
  m.def("add_program_id_to_llvmir", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createGetProgramIdOpToLLVMPass());
  });
  m.def("add_memory_op_to_llvmir", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createMemoryOpToLLVMPass());
  });
  m.def("add_atomic_ops_to_llvmir", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createAtomicOpsToLLVMPass());
  });
  m.def("add_debug_ops_to_llvmir", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createDebugOpsToLLVMPass());
  });
  m.def("add_ukernels_to_onednn_llvmir", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createUkernelOpsToOneDNNLLVMPass());
  });
  m.def("add_ukernels_to_xsmm_llvmir", [](mlir::PassManager &pm) {
    pm.addPass(mlir::triton::gen::createUkernelOpsToXSMMLLVMPass());
  });
  ...
  m.def("add_vector_to_llvmir",
        [](mlir::PassManager &pm, bool reassoc_fp_reduction) {
          ...
          // widely used path for this lowering in GEN case.
  ...
  m.def("add_math_to_vec_lib", [](mlir::PassManager &pm, gen::VecLib lib,
                                  std::set<std::string> gen_features) {
    pm.addPass(mlir::triton::gen::createMathToVecLibPass(lib, gen_features));
  });
...
void init_triton_gen(py::module &&m) {
  ...
  m.def("load_dialects", [](mlir::MLIRContext &context) {
    ...
    registry.insert<mlir::triton::gen::TritonGENDialect,
                    mlir::vector::VectorDialect>();
    mlir::triton::gen::registerTritonOpScalarizeExternalModels(registry);
