...
def _attn_fwd_inner(acc, l_i, m_i, q,  #
    ...
    if STAGE == 1:
        ...
        lo = tl.multiple_of(lo, BLOCK_M) ## ??? it is already a multiple...
    # causal = False
    else:
        lo, hi = 0, N_CTX
...
@triton.jit
def _attn_fwd_inner_sjw(acc, l_i, m_i, q,  #
                        K_block_ptr, V_block_ptr,  #
                        start_m, qk_scale,  #
                        BLOCK_M: tl.constexpr, HEAD_DIM: tl.constexpr, BLOCK_N: tl.constexpr,  #
                        STAGE: tl.constexpr, offs_m: tl.constexpr, offs_n: tl.constexpr,  #
...
def _attn_fwd(Q, K, V, sm_scale, M, Out,  #
    ...
    acc, l_i, m_i = _attn_fwd_inner_sjw(acc, l_i, m_i, q, K_block_ptr, V_block_ptr,  #
                                        start_m, qk_scale,  #
